...
export class CarnesComponent implements OnInit , OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.carnes(this.camera, l).subscribe(
      (res) => {
        this.carnes = res["data"];
        for (var m of this.carnes.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/carnes/' + m['cam_id'] + '/' + m['movie']);
        ...
        let labels = [];
        for (var o of Object.keys(this.carnes.over)) {
          o = o + ":00:00";
          labels.push(this.datepipe.transform(o, "yyyy-M-dd HH:mm"));
        }
        this.source = this.carnes.raw.slice().sort((a, b) => +new Date(b.time) + +new Date(a.time));
            ...
            datasets: [
              {
                ...
                data: Object.values(this.carnes.over),
